...
public class ABMCEjercicio extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "alta":
			...
			if(bandera.equalsIgnoreCase("aAnadir")) {
				...
				try {
					...
					request.setAttribute("mensaje", "Ejercicio a�adido satisfactoriamente.");
					request.getRequestDispatcher("WEB-INF/listaEjercicio.jsp").forward(request, response);
		...
		case "baja":
			...
			if (cant > 0) {
				request.setAttribute("mensaje", "No se puede eliminar el ejercicio, porque est� incluido en "+cant+" grupo m�sculo y/o rutinas");
			}
			else {
